...
const validation = {
    ...
    checkString: (stringVal) => {
        ...
        if (stringVal.length === 0)
            throw `Error: value cannot be empty spaces`;
    ...
    checkEmail: (email) => {
        ...
        email = email.trim();
        if (email.length === 0)
            throw `Error: Email cannot be empty spaces`;
    ...
    checkPassword: (password) => {
        ...
        if (password.length === 0)
            throw `Error: Password cannot be empty spaces`;
    ...
    checkTechnology: (technology) => {
        const allowedTechnologies = [
            'JAVASCRIPT', 'PYTHON', 'JAVA', 'CSHARP', 'CPLUSPLUS', 'RUBY', 'PHP',
            'TYPESCRIPT', 'SWIFT', 'KOTLIN', 'GO', 'RUST', 'HTML', 'CSS', 'SQL',
            'GRAPHQL', 'NODE_JS', 'REACT', 'ANGULAR', 'VUE', 'NEXT_JS', 'SVELTE',
            'TAILWINDCSS', 'BOOTSTRAP', 'AWS', 'GOOGLE_CLOUD', 'ORACLE_CLOUD',
            'DOCKER', 'KUBERNETES', 'MONGODB', 'POSTGRESQL', 'REDIS', 'FIREBASE',
            'GIT', 'GITHUB', 'OTHER'
        ];

        if (!technology)
            throw `Error you must provide a Technology`;
    ...
    checkTechnologies: (technologies) => {
        ...
        if (!technologies)
            throw `Error you must provide Technologies`;
